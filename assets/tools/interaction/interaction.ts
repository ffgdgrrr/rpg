import { __private, _decorator, Component, director, Enum, EventKeyboard, find, input, Input, instantiate, Node, Prefab, resources, SceneAsset, TextAsset } from 'cc';
...
export class interaction extends Component {
    ...
    @property({type:[EventHandler],visible:function(this:interaction){
    ...
    @property({visible: function (this: interaction) {
        return this.interactionEvent === interactionEvent.value;
        }
    })
    ...
    interactionContactFunction(){
            ...
            switch(this.interactionEvent){
                ...
                case interactionEvent.scene:
                    if(this.isEmitFrontScene==true){
                        ...
                        console.log('emitScene',window.localStorage.getItem('FrontScene'))
                    }
        ...
        if(this.启用条件==false){
            ...
            this.localStorage.forEach((value,index)=>{
                if(window.localStorage.getItem(value.Key)==value.Value){
                    if(this.else==true){
                        fuckYou=true
                    }
                }else{
