import { _decorator, Component, Node, find, EventTouch, EventMouse, Collider2D, Contact2DType, IPhysics2DContact, Prefab, UITransform, v3, Camera, Enum, Layers, DynamicAtlasManager, AffineTransform, RigidBody2D, Terrain, Layout, director, instantiate, SpriteFrame, Sprite, inverseLerp } from 'cc';
import { bag } from './bag';
...
import { GZYLocal } from '../../../Player/script/PlayerLocalData';
...
export class mezi extends interaction {
    ...
    pressEvent(){
        if(this.node.parent.name=='content'){
            GZYLocal.getPath()
            let GZYNode=find(GZYLocal.GZYPath)
            console.log(GZYNode.getPathInHierarchy(),'gzyyyy')
            if(GZYNode){
                let GZYScript=GZYNode.getComponent(Player)
                if(GZYScript.contact==true){

                GZYScript.interactionName.getComponents(interaction).forEach(element=>{
                    element.UseMezi(this.node.name)
            })
            }
            console.log(GZYScript.contact) 
        }
            console.log('bagbag')
