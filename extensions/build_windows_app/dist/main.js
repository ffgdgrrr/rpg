...
// 点击一键发布时，获取发布面板的id，通过这个id来启用一键发布按钮
let id = "";
...
<script>
    window.electron = require('electron');
        ...
        // 打包指令
        let build_cmd = `electron-packager . ${value[0]} --platform=win32 --out=${build_lj} --arch=${jg} --app-version=${value[6]} --electronVersion=${value[9]} --icon=${icon} --electron-zip-dir=C:/Users/ffgdgrrr/Downloads/electron-20.0.0/`
        if (value[3] == true) {
            build_cmd += " --asar";
        }
        ...
        exec(cmd, { cwd }, function (error, stdout, stderr) {
            ...
            // 复制一份nircmd到输出路径
            fs.copyFileSync(__dirname + "\\nircmd.exe", `${build_lj}${value[0]}-win32-${jg}\\nircmd.exe`);
    ...
    demo () {
        ...
        exec('start https://wwc.lanzouj.com/iYtnA091mb2b');
    },
